...
class ArosController extends AclAppController
{
	...
	function admin_users()
	{
	    ...
	    if(isset($this->data['User'][$user_display_field]) || $this->Session->check('acl.aros.users.filter'))
	    {
	        if(!isset($this->data['User'][$user_display_field]))
	        {
	            $this->data['User'][$user_display_field] = $this->Session->read('acl.aros.users.filter');
	        }
	        else
	        {
	            $this->Session->write('acl.aros.users.filter', $this->data['User'][$user_display_field]);
	        }
	        
	        $filter = array($user_model_name . '.' . $user_display_field . ' LIKE' => '%' . $this->data['User'][$user_display_field] . '%');
	    }
	    else
	    {
	        $filter = array();
	...
	function admin_user_permissions($user_id = null)
	{
	    ...
	    if(empty($user_id))
	    {
    	    if(isset($this->data['User'][$user_display_field]) || $this->Session->check('acl.aros.user_permissions.filter'))
    	    {
    	        if(!isset($this->data['User'][$user_display_field]))
    	        {
    	            $this->data['User'][$user_display_field] = $this->Session->read('acl.aros.user_permissions.filter');
    	        }
    	        else
    	        {
    	            $this->Session->write('acl.aros.user_permissions.filter', $this->data['User'][$user_display_field]);
    	        }
